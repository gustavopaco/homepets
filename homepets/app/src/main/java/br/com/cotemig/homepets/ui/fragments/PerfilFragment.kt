...
class PerfilFragment : Fragment() {
    ...
    private fun updateUser() {
        ...
        /* RETROFIT AQUI*/
        RetrofitInitializer().serviceAPI().updateUser(token = "Bearer $token", updateUserModel)
    ...
    private fun validaCampos(): Boolean {
        ...
        if (binding.inputNovaSenha1.text.toString().length >= 5 && binding.inputNovaSenha2.text.toString().length >= 5 && (!binding.inputNovaSenha1.text.toString()
                .equals(binding.inputNovaSenha2.text.toString(), false))
        ) {
            MaterialDialog(activity).show {
                title(R.string.erro)
